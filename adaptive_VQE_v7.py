...
for step_round in range(total_round):
    ...
    while abs(optimize_value_old0 - optimize_value_old)>1e-2:
        optimize_value_old0 = optimize_value_old
        result = minimize(energy_filling_check, x0 = known_params, args = (c_new,c1_new,Hamiltonian), method = 'nelder-mead',options={'maxiter':iter_confine2})
        known_params = result.x
        optimize_value_old = energy(known_params, c_new, c1_new, Hamiltonian)
