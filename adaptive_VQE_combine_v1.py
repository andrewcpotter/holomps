...
#, RotGate(qids=[1], n_params=1, fn=qub_z),\
...
for step_round in range(total_round):
    ...
    c_new.assemble()
    c1_new.assemble()
    gate = entangler_choice
    gate1 = entangler_choice
    
    result_new = minimize(energy_part_vary, x0 = sweet_spot_local, args = (known_params,c,c1,gate,gate1,c_new, c1_new,Hamiltonian),\
    method = 'nelder-mead', options={'maxiter':iter_confine2})
    sweet_spot = result_new.x
    optimize_value_old = energy_part_vary(sweet_spot, known_params, c, c1, gate, gate1, c_new, c1_new, Hamiltonian)
    optimize_value_old0 = optimize_value_old + 1
    
    while abs(optimize_value_old0 - optimize_value_old)>1e-3:
        optimize_value_old0 = optimize_value_old
        result_new = minimize(energy_part_vary, x0 = sweet_spot, args = (known_params,c,c1,gate,gate1,c_new, c1_new,Hamiltonian),\
        method = 'nelder-mead', options={'maxiter':iter_confine2})
        sweet_spot = result_new.x
        optimize_value_old = energy_part_vary(sweet_spot, known_params, c, c1, gate, gate1, c_new, c1_new, Hamiltonian)
    ...
    # update known_params:
    a1 = known_params[0: c.n_params]
    a2 = sweet_spot[0: gate.n_params]
    a3 = known_params[c.n_params: c.n_params+c1.n_params]
    a4 = sweet_spot[gate.n_params: gate.n_params+gate1.n_params]
    known_params = np.concatenate((a1, a2, a3, a4), axis=0, out=None)
    # update circuit
    c = c_new
    c1 = c1_new
