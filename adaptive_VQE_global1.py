...
gate_list = [XX_YYGate(qids = [0,2], fn=qub_two), XX_YYGate(qids = [1,3], fn=qub_two)]
...
for step_round in range(total_round):
    ...
    result = basinhopping(energy_filling_check, x0 = params_random,\
    minimizer_kwargs={"method":"nelder-mead","args": (c, c1, Hamiltonian)}, niter = iter_confine2)
    c = c_new
    c1 = c1_new
    known_params = result.x
    optimize_value = energy(known_params, c_new, c1_new, Hamiltonian)
    if optimize_value_old < optimize_value:
        result = basinhopping(energy_filling_check, x0 = params_local_best,\
        minimizer_kwargs={"method":"nelder-mead","args": (c, c1, Hamiltonian)}, niter = iter_confine2)
        known_params = result.x
