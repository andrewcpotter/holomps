...
result = minimize(energy_filling_check, x0=params, args=(c, c1, Hamiltonian), method='nelder-mead',options={'maxiter':iter_confine1})
sweet_spot = result.x
...
for gate_choice in Entangler_pool:
    one_gate_E_copy = one_gate_E.copy()
    for iter_round in range(len(total_round)):
        if iter_round == 0:
            iter_num = total_round[0]
        else:
            iter_num = total_round[iter_round] - total_round[iter_round-1]
        t1 = time.time()
        gate = gate_choice
        gate1 = gate_choice
        if iter_round == 0:
        ...
        one_gate_E_copy.append(optimize_value_new)
        t2 = time.time()
    print(one_gate_E_copy)