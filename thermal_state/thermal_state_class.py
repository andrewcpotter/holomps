...
class thermal_state(object):
    ...
    def network_site_contraction(self, state_type, chi_MPO=None, MPO=None):
        ...
        if state_type == 'random_state' or state_type == 'circuit_MPS':
            ...
            if MPO == None: 
                ...
                for j in range(L):
                    ...
                    tensor = np.tensordot(self[1][j].conj(),self[1][j],axes=[0,0])
                    # reshaping into matrix
                    con_site = np.reshape(np.swapaxes(tensor,1,2),[chi**2,chi**2]) # contraction (transfer) matrix at each site
                    contraction_list.append(con_site)                 
            ...
            # contracted matrices w/ MPO inserted
                ...
                for j in range(L_MPO):
                    ...
                    con_site = np.reshape(np.swapaxes(np.tensordot(self[1][j].conj(),MPO_on_state_reshape,axes=[0,0]),1,2),[chi_tot,chi_tot])
                    contraction_list.append(con_site)
        ...
        # must include MPO (e.g. Hamiltonian MPO)
            ...
            chi_tot = int(np.sqrt(np.trace(tensor,axis1=0,axis2=2).size)) # total bond dimension
            ...
            for j in range(L):
                ...
                contraction_list.append(np.reshape(tensor2,[chi_tot,chi_tot]))
