...
class thermal_state(object):
    ...
    def density_matrix(self, params, L, prob_list=None, bdry_vecs=[None,None]):      
        ...
        if prob_list != None:
            ...
            p_matrix = np.diag(thermal_state.prob_list(None,'density_matrix',self,params))      
        ...
        for j in range(L):
            ...
            W0 = np.tensordot(p_state[1][j],state[1][j],axes=[0,2]) # result: p'_in, p_out, b_out, b_in
            W1 = np.swapaxes(W0,0,2) # changing axis ordering to:  b_out, p_out, p'_in, b_in
    ...
    def free_energy(self, params, state_type, L, H_mat, T, chi_H, prob_list=None, bdry_vecs1=[None,None], bdry_vecs2=[None,None]):
        ...
        if state_type == 'density_matrix':
            ...
            if prob_list != None:
                ...
                S = thermal_state.entropy(thermal_state.prob_list(None,'density_matrix',self,params)) 
        ...
        # for random-holoMPS-based structures:
            ...
            Hamiltonian = thermal_state.network_from_cells(H_mat,'MPO',L,chi_H,None,bdry_vecs2) # Hamiltonian MPO
            S = thermal_state.entropy(thermal_state.prob_list(random_state,'random_state',self,params)) # entropy
