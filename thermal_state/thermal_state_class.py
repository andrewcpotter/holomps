...
class thermal_state(object):
    ...
    def network_from_cells(self, network_type, L, chi_MPO=None, params=None, bdry_vecs=None, method=None, T=None):      
        ...
        if network_type == 'random_state' or network_type == 'circuit_MPS' or network_type == 'circuit_MPO':
            ...
            if network_type == 'random_state': 
                ...
                index_list = [prob_list[j].index(random_list[j]) for j in range(L)]
    ...
    def free_energy(self, params, state_type, L, Hamiltonian, T, chi_H=None, bdry_vecs1=None, bdry_vecs2=None, method=None):
        """
        ...
        if state_type == 'density_matrix':
            ...
            S = thermal_state.entropy(thermal_state.prob_list(self,params,T),L) # entropy
            density_mat = thermal_state.density_matrix(self,params,L,T,bdry_vecs1) # density matrix            
             
            MPO_Hamiltonian = thermal_state.network_from_cells(Hamiltonian,'MPO',L,chi_H,None,bdry_vecs2,'thermal_state_class',T) # Hamiltonian MPO      
            E = thermal_state.expectation_value(density_mat,'density_matrix',chi_H,MPO_Hamiltonian) # energy of system    
            F = (E/L) - T*S # Helmholtz free energy 
            
        # for random-holoMPS-based structures:
        elif state_type == 'random_state':
            
            random_state = thermal_state.network_from_cells(self,'random_state',L,None,params,bdry_vecs1,method,T) # random_state MPS
            MPO_Hamiltonian = thermal_state.network_from_cells(Hamiltonian,'MPO',L,chi_H,None,bdry_vecs2,method,T) # Hamiltonian MPO
            S = thermal_state.entropy(thermal_state.prob_list(self,params,T),L) # entropy
