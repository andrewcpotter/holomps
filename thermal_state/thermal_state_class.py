...
class thermal_state(object):
    ...
    def network_from_cells(self, network_type, L, chi_MPO=None, params=None, bdry_vecs=[None,None]):      
        ...
        --------------
        Inputs:
          --the input assumes either circuit or rank-4 numpy.array tensors--       
          network_type: str
        ...
        Note:
          -For random_state, circuit_MPS and circuit_MPO options, the original circuit with 
           ...
           includes rank-3 numpy.arrays for random_state/circuit_MPS and rank-4 numpy.arrays for
           circuit_MPO.
    ...
    def prob_list(self, L, circuit, params):
        """  
        Returns the list of probabilities of each physical site for random-holographic 
        matrix prodcut states.
        --------------
        Inputs:
          --the input accepts thermal_state_class-based random_state (random-holoMPS)--      
          L: int
             Length (number) of repetitions of unit cell in the main network chain. 
        ...
        # (by comparing with our random state's bulk tensors)
    ...
    def density_matrix(self, params, L, prob_list, bdry_vecs=[None,None]):      
        ...
        Inputs:
          ...
          L: int
             length (number) of repetitions of unit cell in the main network chain.
          prob_list: list 
             List of probability weights of each physical state.
             (the length of prob_list should match the physical leg dimension).
          bdry_vecs: list
            List of left (first element) and right (second element) boundary vectors.
            (set to [None,None] by default which gives left and right boundary vectors = |0>).
        """
        ...
        #TN2 = [np.swapaxes(np.tensordot(state[1][j].conj(),TN1[j],axes=[2,2]),2,3) for j in range(L)]
        
        #TN1 = [np.swapaxes(np.tensordot(p_state[1][j],state[1][j],axes=[0,2]),1,2)for j in range(L)]
        #TN2 = [np.reshape(np.swapaxes(np.tensordot(state[1][j].conj(),np.reshape(TN1[j],[d,chi,d,chi]),axes=[2,2]),2,3),[d,chi**2,d,chi**2]) for j in range(L)]
        
        #TN1 = [np.tensordot(p_state[1][j],state[1][j],axes=[0,2]) for j in range(L)]
        #TN2 = [np.reshape(np.tensordot(state[1][j].conj(),np.reshape(TN1[j],[d,chi,d,chi]),axes=[2,2]),[d,chi**2,d,chi**2]) for j in range(L)]
        
        #TN1 = [np.tensordot(p_state[1][j],state[1][j],axes=[0,2]) for j in range(L)]
        #TN2 = [np.tensordot(state[1][j].conj(),np.reshape(TN1[j],[d,chi,d,chi]),axes=[2,2]) for j in range(L)]
        ...
        density_matrix = [[bvecl_tot],TN2,[bvecr_tot]]
    ...
    def free_energy(self, params, state_type, L, H_mat, T, chi_H, prob_list=None, bdry_vecs1=[None,None], bdry_vecs2=[None,None]):
        ...
        # for random-holoMPS structures:
            ...
            Hamiltonian = thermal_state.network_from_cells(H_mat,'MPO',L,chi_H,None,bdry_vecs2)
            prob_list = thermal_state.prob_list(random_state,L,self,params)
