...
params = rng.uniform(high=2*np.pi,size=c.n_params + d)

def state_free_energy(params, circuit, method):
    """
    ...
    if method == 'thermal_state_class':   
        ...
        sfr = thermal_state.free_energy(circuit,params,'random_state',L,H_mat,T,chi_H,None,bdry_vecs1,bdry_vecs2,'thermal_state_class')  
    
    elif method == 'tenpy':
        H_mat = ising_mpo(J,g,'tenpy',L)
        sfr = thermal_state.free_energy(circuit,params,'random_state',L,H_mat,T,None,None,None,None,'tenpy')
...
# optimization 
result = minimize(state_free_energy,x0=params,args=(c,method),method = TBD,options={'disp': True})
sweet_spot = result.x
F_random = state_free_energy(sweet_spot,c,method)
...
print("Random state method: F_density = {:.8}".format(F_random))
