...
for gate_choice in Entangler_pool:
    ...
    c_new.add_gate("XX_YY", qids=[0, 2], n_params = 1, fn = qub_two)
    c_new.add_gate("ZZ", qids=[0, 2], n_params = 1, fn = qub_two)
    c_new.add_gate("XX_YY", qids=[1, 3], n_params = 1, fn = qub_two)    
    c_new.add_gate("ZZ", qids=[1, 3], n_params = 1, fn = qub_two)
    c_new.gates.append(gate)
    c1_new = Circuit([("qubit", "p", d), ("qubit", "p", d), ("qubit", "b", chimax), ("qubit", "b", chimax)])
    c1_new.add_gate("XX_YY", qids=[0, 2], n_params = 1, fn = qub_two)
    c1_new.add_gate("ZZ", qids=[0, 2], n_params = 1, fn = qub_two)
    c1_new.add_gate("XX_YY", qids=[1, 3], n_params = 1, fn = qub_two)    
    c1_new.add_gate("ZZ", qids=[1, 3], n_params = 1, fn = qub_two)
    c1_new.gates.append(gate1)
    c_new.assemble()
    c1_new.assemble()
...
params_start = rng.uniform(high=2*np.pi, size = c_new.n_params + c1_new.n_params)
result = minimize(energy_filling_check, x0 = params_start, args = (c_new,c1_new,Hamiltonian), method = 'nelder-mead',\
