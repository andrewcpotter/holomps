...
class thermal_state(object):
    ...
    def network_from_cells(self, network_type, L, bdry_vecs=[None,None]):      
        """
        Returns network of finite random holo-Matrix Prodcut State (random_holoMPS), finite 
        holo-MPS (circuit_MPS), or Matrix Prodcut Operator (MPO) of a given tensor.
        --------------
        Inputs:
          --Bulk tensors must be rank-4 numpy.array (consistent with final structure of MPO)--
          --index ordering: physical-out, bond-out, physical-in, bond-in--
          network_type: str
             One of "random_MPS", "circuit_MPS", or "MPO" options.
          L: int
        ...
        Note:
          -For random_MPS and circuit_MPS, the list of bulk tensors would be returned 
          as rank-3 numpy.arrays with index ordering: site, physical, bond-out, bond-in 
          ("in/out" refers to right canonical form ordering).          
        """
        
        # if network_type is set to random_holoMPS:
        if network_type == 'random_MPS':
            
            # tensor dimensions
            tensor = np.swapaxes(self[:,:,0,:],1,2) # change to MPS_based structure
            ...
            tensor_list = [np.swapaxes(self[:,:,j,:],1,2) for j in range(d_r)]  
        ...
        # if network_type is set to holoMPS:
            ...
            # tensor dimensions
            unitary = np.swapaxes(self[:,:,0,:],1,2)
            d_c = unitary[:,0,0].size # physical leg dimension (for holoMPS)
            chi_c = unitary[0,:,0].size # bond leg dimension (for holoMPS)
            
            # bulk tensors of holoMPS structure
            tensors = L*[self]    
            tensor_list1 = [np.swapaxes(tensor[:,:,0,:],1,2) for tensor in tensors]
        ...
        # if network_type is set to MPO: 
            ...
            # tensor dimensions (consistent with rank-4 structure)
            # index ordering consistent with MPO structure
    ...
    def density_matrix(self, L, prob_list, bdry_vecs=[None,None]):      
        ...
        state1 = thermal_state.network_from_cells(self,'MPO',L,bdry_vecs) # constructing state as MPO
        ...
        state2 = thermal_state.network_from_cells(tensor_conj,'MPO',L,bdry_vecs_conj)
    ...
    def free_energy(self, state_type, L, H_mat, T, prob_list=None, bdry_vecs1=[None,None], bdry_vecs2=[None,None]):
        """
        Returns the free energy of a given density matrix/random-holoMPS.
        --------------
        Inputs:
        --the input assumes thermal_state_class-based random-holoMPS, holoMPS, and MPO networks--
        state_type: str
        ...
        if state_type == 'density_matrix':
            ...
            # tensor dimensions (consistent with rank-4 structure)
            # index ordering consistent with MPO structure
            d = self[:,0,0,0].size # physical leg dimension (assumes rank-4 structure)
            chi = self[0,:,0,0].size # bond leg dimension (assumes rank-4 structure)
            if len(prob_list) != d:
                raise ValueError('length of probability list should match the physical dimension')
            
            # constructing density matrix and Hamiltonian (as MPO)
            density_mat = thermal_state.density_matrix(self,L,prob_list,bdry_vecs1)
            Hamiltonian = thermal_state.network_from_cells(H_mat,'MPO',L,bdry_vecs2)
        ...
        # free energy for random-holoMPS state
            ...
            random_state = thermal_state.network_from_cells(self,'random_MPS',L,bdry_vecs1)
            Hamiltonian = thermal_state.network_from_cells(H_mat,'MPO',L,bdry_vecs2)
            ...
            count = [0]*d
            tensor_list = [np.swapaxes(self[:,:,j,:],1,2) for j in range(d)]
            ...
            E = (thermal_state.expectation_value(random_state,L,Hamiltonian)).real
            F = E - T*S # Helmholtz free energy      
        return F
